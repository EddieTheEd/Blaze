...
let headingsArray = Array.from(headingsnodelist);

for (let i = headingsArray.length - 1; i >= 0; i--) {
  if (
    headingsArray[i].id === "pageh1" ||
    headingsArray[i].id === "graphtitle" ||
    headingsArray[i].id === "sidenotetitle" ||
    headingsArray[i].id === "callout-title" ||
    headingsArray[i].id === "backlinkstitle" ||
    headingsArray[i].id === "toctitle"
  ) {
    headingsArray.splice(i, 1);
  }
}
...
if (headingsArray.length > 0){
  ...
  document.getElementById("toccontent").remove();
  let funnytext = document.createElement("p");
  funnytext.innerHTML="No headings here!";
  document.getElementById("toc").appendChild(funnytext);
