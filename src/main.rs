...
fn _format_path(path: &str) -> String {
    // replace \ with /
...
fn compile_markdown(
    ...
    cfg: &Config,
    theme: &String,
    _links: &HashMap<String, Vec<LinkTarget>>,
            ...
            FsThing::File {
                ...
                if path.ends_with(".md") || path.ends_with(".markdown") {
                                    ...
                                    match key {
                                        ...
                                        "draft" => {
                                            ...
                                            if cfg.drafts.use_file_description.unwrap_or(true) {
                                                ...
                                                frontmatter.description = Some(
                                                    cfg.drafts.custom_draft_description.clone(),
                                                );
                                            }
                                        }
                                        "theme" => {
                    ...
                    match &frontmatter.draft {
                        Some(draft) => {
                            if draft == "true" {
                                ...
                                compiled_markdown =
                                    format!("<strong>{}</strong>", cfg.drafts.message);
                            } else {
                    ...
                    if cfg.settings.backlinks.unwrap_or(false) {
                        ...
                        if let Some(backlinks) = backlinks.get(&canonical_path) {
                            for link in backlinks {
                                //println!("--- backlink --- {} from {}",link.path, html_path);
                                let tempkey = "backlink".to_owned()
                                    + &link.path.to_string().replace(".html", "");
                                let tempvalue =
                                    "/".to_owned() + &link.path.to_string().replace("output", "");
                                pathways.insert(tempkey.to_string(), tempvalue.to_string());
                                if let Some(info) = infos.get(&link.path) {
                                    backlinks_partials.push_str(
                                        format!(
                                            r#"{{{{ partial "backlink.html" . ({}, {}, {}, {}) }}}}
"#,
                                            info.title.trim_matches('\'').trim_matches('\"'),
                                            link.path,
                    ...
                    let regex =
                        Regex::new(r#"\{\{\s*partial\s*"(.*)"\s*.\s*(?:\((.*)*\))?\s*\s*\}\}"#)
                            .unwrap();

                    while let Some(caps) = regex.captures(&compiled_html) {
                        ...
                        let mut subs = vec![];
                        if let Some(values) = caps.get(2) {
                            for val in values.as_str().split(',') {
                                subs.push(val.to_string());
                            }
                        }
                        compiled_html =
                            compiled_html.replace(caps.get(0).unwrap().as_str(), partial);
                        for (i, sub) in subs.iter().enumerate() {
                            compiled_html =
                                compiled_html.replace(&format!("{{{{{}}}}}", i).to_string(), sub);
                        }
                    }
                    ...
                    compiled_html =
                        compiled_html.replace("{{date_created}}", &format_date(metadata.created));
                    compiled_html =
                        compiled_html.replace("{{last_modified}}", &format_date(metadata.created));
                    compiled_html =
                        compiled_html.replace("{{last_accesssed}}", &format_date(metadata.created));

                    let callout_regex = Regex::new(
                        r#"<blockquote>\s*<p>\s?\[!(\w+)]\s*([^<]*)\s*((?s).*)\s*</blockquote>"#,
                    )
                    .unwrap();

                    while let Some(caps) = callout_regex.captures(&compiled_html) {
                        let replacement =
                        ...
                        // println!("{}", replacement);
                        compiled_html =
                            compiled_html.replace(caps.get(0).unwrap().as_str(), &replacement);
                    }

                    let link_regex =
                        Regex::new(r#"<a href="((?:../)*)([^."]*)">((?s)[^<]*)</a>"#).unwrap();
                    let dotdotslash_regex = Regex::new(r#"([^/]*)/\.\./"#).unwrap();

                    while let Some(caps) = link_regex.captures(&compiled_html) {
                        ...
                        while let Some(mat) = dotdotslash_regex.find(&full_path) {
                            full_path = full_path.replace(mat.as_str(), "")
                        }
                        //println!("here is a link maybe: {}", full_path);
                        let rootpath = "/".to_owned() + &full_path;
                        pathways.insert(
                            "forwardlink".to_owned() + &full_path.to_string().replace("%20", " "),
                            rootpath.to_string().replace("%20", " "),
                        );
                    ...
                    let latex_block_regex = Regex::new(
                        r#"<pre><code class="language-math math-display">((?s)[^<]*)</code></pre>"#,
                    )
                    .unwrap();

                    while let Some(caps) = latex_block_regex.captures(&compiled_html) {
                    ...
                    let latex_regex = Regex::new(
                        r#"<code class="language-math math-display">((?s)[^<]*)</code>"#,
                    )
                    .unwrap();

                    while let Some(caps) = latex_regex.captures(&compiled_html) {
                        let replacement = format!("${}$", caps.get(1).unwrap().as_str());
                        // println!("{}", &replacement);
                        compiled_html =
                            compiled_html.replace(caps.get(0).unwrap().as_str(), &replacement);
                    ...
                    if cfg.settings.graph.unwrap_or(false) {
                        ...
                        graphdata.push_str(&format!(
                            "{{\"id\":\"{}\",\"link\":\"{}\",\"linktype\":\"var(--root)\"}}",
                            root, rootlink
                        ));
                        ...
                        let globalfilename = "output/global.json".to_string();

                        let mut graphfile =
                            File::create(graphfilename.clone()).expect("Failed to create file");

                        graphfile
                            .write_all(graphdata.as_bytes())
            ...
            FsThing::Directory {
                ...
                compile_markdown(
                    contents, cfg, theme, _links, backlinks, infos, partials, false,
                )
            }
        }
    }
    ...
    for thing in things {
        match thing {
            FsThing::File {
                ...
                if path.ends_with(".html") || path.contains("README.md") {
                    // skip that
                } else {
                    // copy file directly
...
fn walk_directory(path: &str, cfg: &Config, ignore: &str) -> Vec<FsThing> {
    ...
    let entries: Vec<DirEntry> = this_dir.filter_map(|elem| elem.ok()).collect();
    ...
    entries
        ...
        .filter_map(|entry| {
            ...
            if cfg.build.ignore.iter().any(|i| match i {
                Value::String(s) => *s == entry.path().to_str().unwrap().replace('\\', "/"),
                _ => false,
            }) {
            ...
            if entry.path().is_dir() {
                Some(FsThing::Directory {
                    path: entry
                        ...
                        .skip(ignore.len() + 1)
                        .collect::<String>(),
                    contents: walk_directory(
                        entry.path().to_str().expect("should not fail"),
                        cfg,
                        ignore,
                    ),
                    _metadata: metadata,
        ...
        match thing {
            FsThing::File {
                ...
                for cap in link_regex.captures_iter(content) {
                    ...
                    while let Some(mat) = dotdotslash_regex.find(&linked_path) {
                        linked_path = linked_path.replace(mat.as_str(), "")
                    }

                    if linked_path == path.replace('\\', "/") {
                        continue;
                    }
                    if !links.contains_key(&path.replace('\\', "/")) {
                        links.insert(path.clone().replace('\\', "/"), Vec::new());
                    }
                    ...
                    links.insert(path.clone().replace('\\', "/"), outlinks);
                    if !backlinks.contains_key(&linked_path) {
                        backlinks.insert(linked_path.clone(), Vec::new());
                    }
                    let blinks = backlinks.get(&linked_path).unwrap();
                    let mut blinks = blinks.clone();
                    blinks.push(LinkTarget {
                        path: path.replace('\\', "/"),
                    });
                    backlinks.insert(linked_path, blinks);

                    let mut frontmatter = if content.starts_with("---") {
                            ...
                            match c_iter.next().unwrap_or("---").trim() {
                                ...
                                line => {
                                    ...
                                    match key {
                                        ...
                                        "description" => {
                                            frontmatter.description = Some(val.trim().to_string())
                                        }
                                        _ => (),
                                    }
                                }
            ...
            FsThing::Directory {
                ...
                generate_backlinks(contents, _cfg, links, backlinks, infos);
            }
        }
    }
}
...
fn lastmod(folder_path: &str, input: &str) -> String {
    ...
    for entry in entries {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("md") {
            ...
            let naive_date_time =
                NaiveDateTime::parse_from_str(output_str.trim(), "%Y-%m-%d %H:%M:%S %z")
            ...
            outputstring.push_str(&format!(
                ...
                local_date_time.format("%Y-%m-%d %H:%M:%S %z")
            ));
        } else if path.is_dir() {
...
fn main() {
    ...
    if let Err(why) = config_file.read_to_string(&mut config_file_contents) {
        panic!("couldn't read config file ({}): {}", display, why)
    };
    ...
    let partials: HashMap<String, String> = match fs::read_dir(partials_path) {
        Ok(entries) => {
            let entries: Vec<DirEntry> = entries.filter_map(|elem| elem.ok()).collect();
            ...
            for entry in entries {
                ...
                partials.insert(compact_path.to_string(), content);
            }
    ...
    let _ = fs::copy(
        "blazeconfig.toml",
        config.build.output.clone() + "/blazeconfig.toml",
    );
  ...
  \"cleanUrls\": true
    ...
    if config.settings.graph.unwrap_or(false) {
        ...
        let modified_nodes = nodes_objects
            .concat()
            .replace("}{", "}, {")
            .replace("},{", "}, {");
    ...
    let result = lastmod(&config.build.input, &config.build.input)
        .lines()
        .filter(|line| !line.trim().is_empty())
        .collect::<Vec<&str>>()
        .join("\n");

    let mut file = File::create("output/lastmod.txt").expect("Failed to create file");
